...
@bot.slash_command(guild_ids=[config["guildID"]], name="ping", description="Bot's latency.")
async def ping(ctx):
    await ctx.respond(embed = discord.Embed(title = "**Pong!**", description = f"{round(bot.latency * 1000)} ms", color = 0x4598d2))
    
    
@bot.slash_command(guild_ids=[config["guildID"]], name="restock", description="Allows one to restock nitro tokens.")
